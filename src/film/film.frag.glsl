#version 300 es
// The film treatment (§7.5), GLSL 300 es. Effects in order:
// 1 sub frame blend · 2 depth parallax · 3 velocity dispersion · 4 DoF ·
// 5 barrel warp · 6 palette grade · 7 animated grain · 8 vignette + wash.
...
void main() {
...
#ifndef LOW_QUALITY
  // EFFECT 3 · velocity chromatic dispersion, max ~2.2px separation
  float disp = abs(uVelocity) * (2.2 / uResolution.x);
  vec3 col;
  ...
  // EFFECT 6 · grade into the palette
  col = mix(INK, col, 0.94);                                  // lift blacks to ink
  col = mix(col, BONE, smoothstep(0.82, 1.0, luma(col)) * 0.10);
  col = mix(vec3(luma(col)), col, 0.88);                      // desaturate 12 percent
  col = mix(col, EMBER, smoothstep(0.72, 0.98, luma(col)) * 0.07);
  col = pow(col, vec3(1.04));                                 // slight contrast

  // EFFECT 7 · animated grain, constant in screen pixels
  float g = hash(gl_FragCoord.xy + fract(uTime) * 100.0);
  col += (g - 0.5) * 0.03;

  // EFFECT 8 · vignette, then wash
  float vig = 1.0 - smoothstep(0.45, 1.05, length(vUv - 0.5) * 1.42);
  col = mix(INK, col, mix(0.45, 1.0, vig));
  col = mix(INK, col, uWash);
